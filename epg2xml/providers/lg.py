...
***********************************************************************
    ...
    def __epgs_of_day(self, channelid: str, data: list) -> List[EPGProgram]:
        ...
        for p in data:
            ...
            _epg.extend_extras([p["brdPgmRsolNm"]])  # 화질
            if p["subtBrdYn"] == "Y":
                _epg.extend_extras(["자막"])
            if p["explBrdYn"] == "Y":
                _epg.extend_extras(["화면해설"])
            if p["silaBrdYn"] == "Y":
                _epg.extend_extras(["수화"])
            ...
            if P_CATE[p["urcBrdCntrTvSchdGnreCd"]]:
                _epg.extend_categories([P_CATE[p["urcBrdCntrTvSchdGnreCd"]]])
            _epgs.append(_epg)
        return _epgs
