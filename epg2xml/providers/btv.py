    ...
    def request(self, url, params, method="GET", output="json"):
        ...
        try:
            ...
            if data["statusCode"].lower() == "ok":
                ...
                raise ValueError(f"유효한 응답이 아닙니다: {data['statusMessage']}")
        except Exception:
            log.exception("Exception while requesting data for %s with %s", url, params)
        return ret
...
%s는 현재 시간부터 당일 자정까지만 EPG를 제공하고 있습니다.
                ...
                for program in channels[_ch.svcid]:
                    try:
                        ...
                        if matches:
                            ...
                            _prog.rebroadcast = bool(matches.group(5))
                        ...
                        log.exception("해당 날짜에 EPG 정보가 없거나 없는 채널입니다: %s %s", day.strftime("%Y%m%d"), _ch)
                if not lazy_write:
