    ...
    def get_svc_channels(self):
        ...
        for c in channelcate:
            ...
            if not soup.find_all(attrs={'disp-attr': "B3T"}):
                continue
            all_channels = [str(x.text.strip()) for x in soup.select('div[id="channelNaviLayer"] > div[class="layer_tv layer_all scroll"] > div > ul > li')]
            if not all_channels:
    ...
    def get_programs(self, lazy_write=False):
        ...
        for idx, _ch in enumerate(self.req_channels):
            ...
            for nd, day in enumerate(days):
                ...
                if len(hours) != 24:
                    log.warning(f'24개의 시간 행이 있어야 합니다: {_ch}, 현재: {len(hours)}')
                    break
